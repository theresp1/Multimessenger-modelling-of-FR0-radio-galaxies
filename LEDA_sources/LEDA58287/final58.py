...
#Paliya data:  ##TODO: Put this into an input file
# x1 = 2.0404  Gev    y1 = 1.3751**(-13)
# x2 = 8.5002  Gev    y2 = 1.8894*10**(-13)
# x3 = 35.464  Gev    y3 = 1.7367*10**(-13)
# xu = 146.39  Gev    yu = 3.5169*10**(-13)  #Upper limit 
#x1_error = [0.997, 4.1438]         y1_error = [8.2024*10**(-14), 1.8403*10**(-13)]
#x2_error = [4.1903, 17.245]        y2_error = [1.123*10**(-13), 2.6573*10(-13)]
#x3_error = [17.465, 72.537]        y3_error = [4.4767*10**(-14), 3.0388*10**(-13)] 
#xu_error = [72.575, 295.56]        yu_error = [1.8766*10**(-13),0] 



x1_p = (2.0412 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
x2_p = (8.5067 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
x3_p = (35.481 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
xu_p = (146.72 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
...
x2_lower_error = (4.1903 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
x3_lower_error = (17.465 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
xu_lower_error = (72.465 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
x1_upper_error = (4.1438 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
x2_upper_error = (17.245 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
x3_upper_error = (72.537 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
xu_upper_error = (295.56 * u.GeV).to(u.Hz, equivalencies=u.spectral()).value
...
#y_error_min_p = np.array([ y1_p -8.2024*10**(-14), y2_p -1.123*10**(-13),y3_p - 4.4767*10**(-14), yu_p - 1.8766*10**(-13)] )
y_error_min_p = np.array([ y1_p -8.2024*10**(-14), y2_p -1.123*10**(-13),y3_p - 4.4767*10**(-14), 0] )

y_error_max_p = np.array([1.8403*10**(-13)- y1_p, 2.6573*10**(-13) - y2_p,3.0388*10**(-13)-y3_p,0] )

xerror_p = [x_error_min_p,x_error_max_p]
yerror_p = [y_error_min_p, y_error_max_p]

isup_lim_p = np.array([False,False,False,True])
up_lim_array_p = np.array([0,0,0,3.5169*10**(-13)])


##additional X-ray data: 
#y_X_1            = 4.8492e-14
#y_X_upp_error_1  = 2.3655e-14
#x_X_1            = (5.567e17 *u.Hz).value
#x_X_err_n_1      = (0.5* u.keV).to(u.Hz, equivalencies = u.spectral()).value
#x_X_err_p_1      = (7* u.keV).to(u.Hz, equivalencies = u.spectral()).value

y_X_2            = 1.2534e-14
y_X_upp_error_2  = 6.5520e-15
x_X_2            = (2.22e17 *u.Hz).value
x_X_err_n_2      = (0.5* u.keV).to(u.Hz, equivalencies = u.spectral()).value
x_X_err_p_2      = (1.2* u.keV).to(u.Hz, equivalencies = u.spectral()).value

y_X_3            = 1.56e-9* 3.77e17 * 10**-(23)
y_X_upp_error_3  = 4.3365e-15
x_X_3            = (3.77e17 *u.Hz).value
x_X_err_n_3      = (1.2* u.keV).to(u.Hz, equivalencies = u.spectral()).value
x_X_err_p_3      = (2.0* u.keV).to(u.Hz, equivalencies = u.spectral()).value


# y_X     = np.array([y_X_1,y_X_2,y_X_3])
# x_X     = np.array([x_X_1,x_X_2,x_X_3])
# x_X_min = np.array([x_X_err_n_1,x_X_err_n_2,x_X_err_n_3])
# x_X_max = np.array([x_X_err_p_1,x_X_err_p_2,x_X_err_p_3])
# y_X_min = np.array([y_X_upp_error_1,y_X_upp_error_2,y_X_upp_error_3])
# y_X_max = np.array([y_X_upp_error_1,y_X_upp_error_2,y_X_upp_error_3])

y_X     = np.array([y_X_2,y_X_3])
x_X     = np.array([x_X_2,x_X_3])
x_X_min = np.array([x_X_err_n_2,x_X_err_n_3])
x_X_max = np.array([x_X_err_p_2,x_X_err_p_3])
y_X_min = np.array([y_X_upp_error_2,y_X_upp_error_3])
y_X_max = np.array([y_X_upp_error_2,y_X_upp_error_3])

isup_lim_X    = np.array([False,True])
up_lim_array_X = np.array([0,y_X_3])
...
## Read the data 
with open('input/LEDA58287.txt') as f:
    while True:
        line = f.readline()
        if not line:
            break
...
frequency_array       = np.append(frequency_array, x_X)
frequency_array       = np.append(frequency_array, x_p)


Nufnu_array           = np.asarray(Nufnu,dtype = float) #+ y_p
Nufnu_array           = np.append(Nufnu_array,y_X)
Nufnu_array           = np.append(Nufnu_array,y_p)

e_min_values          = np.asarray(e_min,dtype = float)
e_min_values          = np.append(e_min_values,x_X_min)
e_min_values          = np.append(e_min_values,x_error_min_p)

e_max_values          = np.asarray(e_max,dtype = float)
e_max_values          = np.append(e_max_values,x_X_max)
e_max_values          = np.append(e_max_values,x_error_max_p)

isup_lim_array          = np.asarray(isup_lim,dtype = bool)
isup_lim_array          = np.append(isup_lim_array, isup_lim_X)
isup_lim_array          = np.append(isup_lim_array, isup_lim_p)

uplim_array           = np.asarray(up_array,dtype=float)
uplim_array           = np.append(uplim_array,up_lim_array_X)
uplim_array           = np.append(uplim_array,up_lim_array_p)


#print("up_lim_array", up_lim_array)
#print("size uplim array: ", np.size(up_lim_array))
#print("size array: ", np.size(e_max_values))

Nufnu_error_array_min     = np.asarray(Nufnu_error,dtype = float) #+yerror_p
Nufnu_error_array_min = np.append(Nufnu_error_array_min,y_X_min) 
Nufnu_error_array_min = np.append(Nufnu_error_array_min,y_error_min_p) 


Nufnu_error_array_max    = np.asarray(Nufnu_error,dtype = float) #+yerror_p
Nufnu_error_array_max = np.append(Nufnu_error_array_max,y_X_max)
Nufnu_error_array_max = np.append(Nufnu_error_array_max,y_error_max_p)


#print("err: ", np.size(Nufnu_error_array_min), np.size(Nufnu_error_array_max))
#print("x and y values: ",np.size(frequency_array ),np.size(Nufnu_array))
...
# #table["is_ul"]        = [True,True,True,True,True,False,False,False,False,False,False,False,False,True,False]         #for when we reomve datapoint 11. 
...
B       = 0.1* u.G
# disk
#L_disk  = 9e44 * u.Unit("erg s-1")  # disk luminosity
# M_BH    = 0.1e10 * M_sun   #1e8-1e9 range leave it free 
# eta     = 1 / 12
# m_dot   = (L_disk / (eta * c ** 2)).to("g s-1")   #(L_disk / (eta * c ** 2)).to("g s-1") #about 40Msolar
# R_g     = ((G * M_BH) / c ** 2).to("cm")
# R_in    = 35 * R_g
# R_out   = 200* R_g #too much with 1000?  
L_disk  = 0.8e45 * u.Unit("erg s-1")  # disk luminosity
...
# - EED
agnpy_ssc.log10_k_e.quantity =-3.7359e+00
agnpy_ssc.log10_k_e.frozen = True
agnpy_ssc.p1.quantity =  1.7
...
agnpy_ssc.log10_gamma_min.quantity = np.log10(10)   #2.0792e+00 
agnpy_ssc.log10_gamma_min.frozen = True
agnpy_ssc.log10_gamma_max.quantity =  5.4912e+00  #np.log10(3.0*10**4)    #5.3802e+00    #np.log10(6.4e5)
agnpy_ssc.log10_gamma_max.frozen = True



# define model
model = SkyModel(name="LEDA58287", spectral_model=agnpy_ssc)
...
plt.ylim(1e-20,1e-8)
plt.savefig("LEDAoutput/LEDA58287_fit4.png")
...
total_sed = synch_sed + ssc_sed + dt_bb_sed # + sed_ec_SSDisk  #     + dt_bb_sed #+ ec_dt_sed + disk_bb_sed
...
plt.savefig("LEDAoutput/LEDA58287_fit4.1.png")
plt.show()
...
print("M_edd", M_edd)
print("mdot.value(M):",agnpy_ssc.m_dot.value)
