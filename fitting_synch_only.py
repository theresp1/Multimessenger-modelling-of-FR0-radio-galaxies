...
class AgnpySSC(SpectralModel):
    ...
    def evaluate(
        ...
        sed = sed_synch  # + sed_ssc  #dont need sed_ssc if we are only sitting synchotron ehere 
        return (sed / energy ** 2).to("1 / (cm2 eV s)")

df_1                = pd.read_csv('input/fitting_s.csv')
...
table["is_ul"]        = [True,True,True,True,True,False,False,False,False]
...
agnpy_ssc.p2.quantity = 4.8
agnpy_ssc.log10_gamma_b.quantity = 4.5
agnpy_ssc.log10_gamma_min.quantity = np.log10(500)
agnpy_ssc.log10_gamma_min.frozen = False
agnpy_ssc.log10_gamma_max.quantity = np.log10(1e5)
...
agnpy_ssc.d_L.frozen = True
# - blob parameters
agnpy_ssc.delta_D.quantity = 2
agnpy_ssc.delta_D.frozen = True
agnpy_ssc.log10_B.quantity = np.log10( 1/(2) * (1+z) * (v_s**2)/(2.8*10**6* v_c)  )
...
agnpy_ssc.log10_gamma_min.frozen = False
agnpy_ssc.log10_gamma_max.quantity = np.log10(0.9e6)
agnpy_ssc.log10_gamma_max.frozen = False
...
plt.savefig("output_syn_only/Fit4.png")
plt.show()

#agnpy_ssc.covariance.plot_correlation()
#plt.savefig("output_new_test_data/Fit2_correlation.png")
plt.show()
