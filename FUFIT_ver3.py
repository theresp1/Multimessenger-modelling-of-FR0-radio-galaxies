...
y_err_stat = (flux_points.table["e2dnde_errn"] + flux_points.table["e2dnde_errp"])/2  ### THIS WAS PREVIOUSLT 0, THATS A MISTAKE! Whichn does indeed affect chi2
print("initial stat error",y_err_stat)

unc = 0.1
y_err_stat[0] = y[0] * unc
y_err_stat[1] = y[1] * unc
y_err_stat[2] = y[2] * unc
y_err_stat[3] = y[3] * unc
y_err_stat[4] = y[4] * unc
y_err_stat[5] = y[5] * unc
y_err_stat[6] = y[6] * unc
y_err_stat[7] = y[7] * unc
y_err_stat[8] = y[8] * unc
#y_err_stat[9] = y[9] * unc
y_err_stat[10] = y[10] * unc
...
# - blob parameters
agnpy_ssc.delta_D.quantity = 1.35
agnpy_ssc.delta_D.frozen = True
agnpy_ssc.log10_B.quantity =  np.log10((1.51))
...
agnpy_ssc.p2.quantity = 4.1 
agnpy_ssc.p1.frozen = True
agnpy_ssc.p2.frozen = True
agnpy_ssc.log10_gamma_b.quantity = np.log10(2490) 
agnpy_ssc.log10_gamma_b.frozen = True
agnpy_ssc.log10_gamma_min.quantity = np.log10(120) 
agnpy_ssc.log10_gamma_min.frozen = True
agnpy_ssc.log10_gamma_max.quantity = np.log10(3*10**4)  -0.1
agnpy_ssc.log10_gamma_max.frozen = True


# define model
model = SkyModel(name="Tol_1326-379", spectral_model=agnpy_ssc)
...
#plt.savefig("FU_best_gmax/Fit6.png")
#plt.show()

#agnpy_ssc.covariance.plot_correlation()
#plt.savefig("FU_FIT4/Base_correlation.png")
plt.show()
