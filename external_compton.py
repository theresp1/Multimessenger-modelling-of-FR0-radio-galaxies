...
class AgnpyEC(SpectralModel):
    ...
    log10_B = Parameter("log10_B", 0.0, min=-3, max=1.0)
    ...
    def evaluate(
        ...
        sed = sed_synch + sed_ssc + sed_bb_disk + sed_bb_dt + sed_ec_SSDisk + sed_ec_dt 
...
B       = 0.2 * u.G
# disk
L_disk  = 1.7e43 * u.Unit("erg s-1")  # disk luminosity
M_BH    = 0.9e9 * M_sun   #1e8-1e9 range leave it free 
eta     = 1 / 12
...
agnpy_ec.log10_gamma_b.quantity = np.log10(2600)
agnpy_ec.log10_gamma_b.frozen = True
agnpy_ec.log10_gamma_min.quantity = np.log10(50)
agnpy_ec.log10_gamma_min.frozen = True
agnpy_ec.log10_gamma_max.quantity = np.log10(5.9e5)
...
ec_SSD      = ExternalCompton(blob,disk)
# SEDs
